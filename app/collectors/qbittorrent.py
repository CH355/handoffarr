...
def is_dead_torrent(torrent: dict[str, Any]) -> bool:
    ...
    # Seed counters are the fallback only when qBittorrent omitted availability;
    # a positive availability value is direct evidence that pieces exist.
    no_available_source = (
...
def delete_torrents(
    ...
    try:
        with httpx.Client(timeout=15.0) as client:
            if not _login(
                client, base_url, svc.get("username", ""), svc.get("password", "")
            ):
                return {"ok": False, "error": "login failed"}
...
def collect(config: Config) -> int:
    ...
    try:
        with httpx.Client(timeout=15.0) as client:
            ...
            db.replace_qbittorrent_torrents(normalized_snapshot)
